  ...
  if (data & 0x80)
      ...
      if (!(PSG.Current_Register & 1))
	{
	  ...
	  if (PSG.Current_Channel != 3)
		...
		(PSG.
		 Register[PSG.
			  Current_Register] & 0x0F) | ((data & 0x3F) << 4);
...
void
PSG_Update_SIN (int **buffer, int length)
{
  int i, j, out;
...
void
PSG_Update (int **buffer, int length)
{
  int i, j;
  int cur_cnt, cur_step, cur_vol;
