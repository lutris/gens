; hq2x filter modification for Gens
; by Maxim in Dec 2003
; I changed the parameters to the (pure asm) hq2x function
; to match the format for all blitters,
; and made it able to handle the format of the data passed by 
; Gens (because Gens uses a static array with a variable width
; section used).
; Also added (elsewhere):
; - Blit_HQ2x_InitLUTs() in hqfilters.c
; - Call to that in main intialisation
; - header in blit.h
; Note that this ASM code needs the -O1 flag to compile.
...
.nexty
    ...
    add     edi,ebx 
    mov ebx,320          ; added - move dest pointer on a bit more
	sub ebx,[ebp+Xres]   ;         amount = (320-x)*4
	shl ebx,2            ; 
	add edi,ebx          ;
    dec     dword[linesleft]
    jz      .fin
    mov     ebx,[ebp+Xres]
    shl     ebx,1
    add ebx,[ebp+offset] ; added - adjust relative offsets of lines
