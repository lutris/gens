...
void about_window_show(void)
{
	...
	if (about_wndclass.lpfnWndProc != about_window_wndproc)
	{
		...
		pRegisterClass(&about_wndclass);
	}
	
	// Create the window.
	about_window = pCreateWindow("about_window", "About " GENS_APPNAME,
...
static void about_window_create_child_windows(HWND hWnd)
{
	...
	{
		// Gens logo
		imgGensLogo = pCreateWindow(WC_STATIC, NULL, WS_CHILD | WS_VISIBLE | SS_BITMAP,
		...
		pSendMessage(imgGensLogo, STM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hbmpGensLogo);
	}
	else
	{
		// "ice" timer
	...
	lblGensTitle = pCreateWindow(WC_STATIC, about_window_title, WS_CHILD | WS_VISIBLE | SS_CENTER,
					128, 8, (ABOUT_WINDOW_WIDTH-128), 32+ABOUT_WINDOW_GIT_HEIGHT,
					hWnd, NULL, ghInstance, NULL);
	SetWindowFont(lblGensTitle, fntTitle, true);
	
	lblGensDesc = pCreateWindow(WC_STATIC, about_window_description, WS_CHILD | WS_VISIBLE | SS_CENTER,
	...
	// Tab control.
	tabInfo = pCreateWindow(WC_TABCONTROL, NULL, WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | WS_TABSTOP,
	...
	HWND grpTabContents;
	grpTabContents = pCreateWindow(WC_BUTTON, NULL, WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
	...
	// Tab contents.
	lblTabContents = pCreateWindow(WC_STATIC, about_window_copyright, WS_CHILD | WS_VISIBLE | SS_LEFT,
					8, 16,
...
static LRESULT CALLBACK about_window_grpTabContents_wndproc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	return pCallWindowProc(grpTabContents_old_wndproc, hWnd, message, wParam, lParam);
}
...
static void CALLBACK about_window_callback_iceTimer(HWND hWnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	pSendMessage(about_window, WM_PAINT, 0, 0);
