...
// MDP Host Services.
static const mdp_host_t *mdp_render_scale4x_host_srv = NULL;
 ...
 */
int MDP_FNCALL mdp_render_scale4x_init(const mdp_host_t *host_srv)
{
	// Save the MDP Host Services pointer.
	mdp_render_scale4x_host_srv = host_srv;
	
	// Register the renderer.
	return mdp_render_scale4x_host_srv->renderer_register(&mdp, &mdp_render);
}
...
int MDP_FNCALL mdp_render_scale4x_cpp(const mdp_render_info_t *render_info)
{
	if (!render_info)
