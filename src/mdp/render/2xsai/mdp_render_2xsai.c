...
// MDP Host Services.
static const mdp_host_t *mdp_render_2xsai_host_srv = NULL;
 ...
 */
int MDP_FNCALL mdp_render_2xsai_init(const mdp_host_t *host_srv)
{
	if (!host_srv)
	...
	return mdp_render_2xsai_host_srv->renderer_register(&mdp, &mdp_render);
}
...
int MDP_FNCALL mdp_render_2xsai_end(void)
{
	...
	mdp_render_2xsai_host_srv->renderer_unregister(&mdp, &mdp_render);
 ...
 * @return MDP error code.
 */
int MDP_FNCALL mdp_render_2xsai_cpp(const mdp_render_info_t *render_info)
{
	if (!render_info)
