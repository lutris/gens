...
// MDP Host Services.
static const mdp_host_t *mdp_render_scanline_25_host_srv = NULL;
 ...
 */
int MDP_FNCALL mdp_render_scanline_25_init(const mdp_host_t *host_srv)
{
	// Save the MDP Host Services pointer.
	mdp_render_scanline_25_host_srv = host_srv;
	
	// Register the renderer.
	return mdp_render_scanline_25_host_srv->renderer_register(&mdp, &mdp_render);
}
 ...
 * @return MDP error code.
 */
int MDP_FNCALL mdp_render_scanline_25_end(void)
{
	if (!mdp_render_scanline_25_host_srv)
		return MDP_ERR_OK;
	
	// Unregister the renderer.
	mdp_render_scanline_25_host_srv->renderer_unregister(&mdp, &mdp_render);
...
int MDP_FNCALL mdp_render_scanline_25_cpp(const mdp_render_info_t *render_info)
{
	if (!render_info)
