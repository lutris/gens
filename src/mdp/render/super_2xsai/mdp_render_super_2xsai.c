...
#include "mdp_render_super_2xsai_plugin.h"
...
// x86 asm versions.
#ifdef GENS_X86_ASM
#include "mdp_render_super_2xsai_x86.h"
#endif /* GENS_X86_ASM */

// MDP Host Services.
static const mdp_host_t *mdp_render_super_2xsai_host_srv = NULL;


/**
 * mdp_render_super_2xsai_init(): Initialize the 2xSaI rendering plugin.
 * @return MDP error code.
 */
int MDP_FNCALL mdp_render_super_2xsai_init(const mdp_host_t *host_srv)
{
	if (!host_srv)
		return -MDP_ERR_INVALID_PARAMETERS;
	
	// Save the MDP Host Services pointer.
	mdp_render_super_2xsai_host_srv = host_srv;
	
	// Register the renderer.
	return mdp_render_super_2xsai_host_srv->renderer_register(&mdp, &mdp_render);
}


/**
 * mdp_render_super_2xsai_end(): Shut down the 2xSaI rendering plugin.
 * @return MDP error code.
 */
int MDP_FNCALL mdp_render_super_2xsai_end(void)
{
	if (!mdp_render_super_2xsai_host_srv)
		return MDP_ERR_OK;
	
	// Unregister the renderer.
	mdp_render_super_2xsai_host_srv->renderer_unregister(&mdp, &mdp_render);
...
/**
 * mdp_render_super_2xsai_cpp(): 2xSaI rendering function.
 * @param render_info Render information.
 * @return MDP error code.
 */
int MDP_FNCALL mdp_render_super_2xsai_cpp(const mdp_render_info_t *render_info)
{
	if (!render_info)
